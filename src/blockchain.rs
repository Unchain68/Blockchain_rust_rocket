...
impl Blockchain {
    ...
    pub fn address_transactions_to_pending_transaction(&mut self, transaction_obj : Transaction) -> usize {
        self.pending_transaction.push(transaction_obj);
        self.get_last_block().index + 1
    }

    pub fn hash_block(&self, previous_blockhash: &String, current_block_data: &Block, nonce: &u128) -> String {
    ...
    pub fn proof_of_work(&self, previous_blockhash: &String , current_block_data: &Block) -> u128 {
        let mut nonce = 0;
    ...
    pub fn chain_is_valid(&self, blockchain : &Vec<Block>) -> bool {
        ...
        for i in 1..blockchain.len(){
            ...
            let block_hash = self.hash_block(&prev_block.hash, &current_block, &current_block.nonce);
